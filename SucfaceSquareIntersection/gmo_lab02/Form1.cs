...
namespace gmo_lab02
{
    public partial class Form1 : Form
    {
		...
		public class Plane
		{
			...
			public Plane(float[] _mas) : this(_mas[0], _mas[1], _mas[2], _mas[3]) { }
			public Plane(double a, double b, double c, double d): this((float)a,(float)b,(float)c,(float)d) {}
			
			public float GetX(float y, float z)
			{
				if(A!=0) return -(D+B*y+C*z)/A;
				else return 9999;
			}
			public float GetY(float x, float z)
			{
				if(B!=0) return -(D+A*x+C*z)/B;
				else return 9999;
			}
			public float GetZ(float x, float y)
			{
				if(C!=0) return -(D+B*y+A*x)/C;
				else return 9999;
			}
		}
		...
		private void curve_int_plane_points(List<Point3d> Curve, Plane tPlane, ref List<Point3d> res)
		{
			for(int i=0; i< (Curve.Count-1); i++)
			{
				Point3d P4 = Curve[i];
                Point3d P5 = Curve[i+1];
				float t = (tPlane.A*P4.x + tPlane.B*P4.y + tPlane.C*P4.z + tPlane.D)/(tPlane.A*(P4.x-P5.x)+tPlane.B*(P4.y-P5.y)+tPlane.C*(P4.z-P5.z));
				if (0 <= t && t <= 1)
				{
		...
		private bool segment_int_shape(List<List<Point3d>> tShape, Point3d P4, Point3d P5, ref Point3d IntPoint)
		{
			...
			if(res.Count>0) { IntPoint = res[0]; return true; }
			else return false;
		}
		...
		private void getIntersectPoints(List<List<Point3d>> tShape, Plane tPlane, ref List<List<Point3d>> res)
		{
			int LEHGTH = tShape.Count / 2;
			
			// 4 краевые кривые:
			List<Point3d> l1 = tShape[0];
			List<Point3d> l2 = tShape[LEHGTH-1];
			List<Point3d> l3 = tShape[LEHGTH];
			List<Point3d> l4 = tShape[tShape.Count-1];
			
			//находим краевые точки:
			List<Point3d> targetPoints = new List<Point3d>();
			curve_int_plane_points(l1,tPlane,ref targetPoints);
			curve_int_plane_points(l2,tPlane,ref targetPoints);
			curve_int_plane_points(l3,tPlane,ref targetPoints);
			curve_int_plane_points(l4,tPlane,ref targetPoints);
			
			//определяем шаги по X,Y идем на возростание по Х во избежание дублирования кривых пересечения:
			float dx = (float)numericUpDownSX.Value;
            float dy = (float)numericUpDownSY.Value;
			foreach(Point3d i in targetPoints)
			{
                Point3d j = i;
                List<Point3d> t = new List<Point3d>();
				t.Add(j);
				Point3d t_n = new Point3d(0,0,0);
				//private bool segment_int_shape(List<List<Point3d>> tShape, Point3d P4, Point3d P5, ref Point3d IntPoint)
				while(segment_int_shape(tShape, new Point3d(j.x+dx,j.y+dy,tPlane.GetZ(j.x+dx,j.y+dy)), new Point3d(j.x+dx,j.y-dy,tPlane.GetZ(j.x+dx,j.y-dy)),ref t_n))
				{
					t.Add(t_n);
					j = t_n;
				}
        ...
        private List<Point3d> sq;

        protected ShapeB Figure;

        protected Plane CurPlane;

        //protected CurveB Curve;
		
		protected List<List<Point3d>> IntersectPoints;
        ...
        public Form1()
        {
			...
			*/
			...
			CurPlane = new Plane((float)numericUpDownA.Value,(float)numericUpDownB.Value,(float)numericUpDownC.Value,(float)numericUpDownD.Value);
		...
		private void Build()
		{
			...
			IntersectPoints = new List<List<Point3d>>();
			getIntersectPoints(kon_shape_Or, CurPlane, ref IntersectPoints);
		}
